...
/// WHY validate here and not only in UploadService?
/// UploadService is generic and reusable across many features.
/// This validator is specific to the profile picture context —
/// it enforces business rules (allowed types, max size) at the
/// MediatR pipeline level so invalid requests never reach the service layer.
/// </summary>
public class UploadProfilePictureCommandValidator : AbstractValidator<UploadProfilePictureCommand>
{
    ...
    public UploadProfilePictureCommandValidator()
    {
        ...
        // Only run the following rules if File is not null
        When(x => x.File != null, () =>
        {
