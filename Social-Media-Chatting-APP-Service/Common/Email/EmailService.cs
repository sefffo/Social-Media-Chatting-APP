    ...
    ) : IEmailService
{
    ...
    public async Task SendAsync(string to, string subject, string body, bool isHtml = true)
    {
        ...
        try
        {
            logger.LogInformation("[Email] Connecting to SMTP {Host}:{Port} (UseSsl={UseSsl})...",
                settings.Host, settings.Port, settings.UseSsl);

            // SecureSocketOptions.StartTls forces STARTTLS on port 587 explicitly.
            // Passing UseSsl=false maps to Auto which sometimes negotiates plain — this is safer.
        ...
        catch (AuthenticationException authEx)
        {
            logger.LogError(authEx,
                ...
                settings.Username);
            throw; // rethrow so the background worker logs it too
        }
